...
- **Race scenario:** On the `IS_PYBIND_2_13_PLUS` path, `gil_safe_call_once_and_store` is used, which is thread-safe. On older pybind, C++ static local initialization guarantees thread-safe init, but `py::module_::import` called during init acquires Python's import lock. If two threads race to init simultaneously, the C++ runtime blocks one while the other does the import. This is safe as long as no circular dependency exists.
