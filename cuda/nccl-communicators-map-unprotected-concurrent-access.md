...
- **Note:** This is a pre-existing bug, not specific to free-threading. All Python NCCL bindings in `python_nccl.cpp` release the GIL (`gil_scoped_release`) before calling into the C++ functions that access `_communicators`. Two threads calling e.g. `torch.cuda.nccl.broadcast` concurrently will race on the map even with the GIL enabled.
- **Practical risk:** This is the legacy `torch.cuda.nccl` API, not the primary ProcessGroup NCCL backend. Multi-threaded use is uncommon but not impossible.
